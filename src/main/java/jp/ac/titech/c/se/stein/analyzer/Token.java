...
/**
 * A single leaf token of a source file: its text, its grammar-derived type (the same typing the
 * FinerGit token sequence uses) or null when the tokenizer has no grammar to take a type from, its
 * 1-based start line and column, its character offset into the decoded
