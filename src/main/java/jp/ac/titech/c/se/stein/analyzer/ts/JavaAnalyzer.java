...
public class JavaAnalyzer extends QueryAnalyzer {
    ...
    protected Signature signature(final Element.Kind kind, final TSNode node, final Captures captures) {
        if (kind != Element.Kind.METHOD) {
            return Signature.of(textOf(captures.get("name")));
        }
        return new Signature(textOf(node.getChildByFieldName("name")), methodTypeParameters(node), methodParameters(node));
    }
     ...
     */
    protected List<String> methodTypeParameters(final TSNode node) {
     ...
     */
    protected List<String> methodParameters(final TSNode node) {
     ...
     * the generated names match JdtAnalyzer.
     */
    ...
    /**
     * The escaping JdtAnalyzer's method naming applies to type names.
     */
