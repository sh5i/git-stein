...
/**
 * A {@link TreeSitterModel} whose element <em>detection</em> is declarative: a subclass supplies a
 ...
 */
public abstract class QueryModel extends TreeSitterModel {
    private final TSQuery query;

    protected QueryModel(final String filename, final SourceText text, final TSNode treeRoot, final TSQuery query) {
     ...
     */
    protected abstract Signature signature(Element.Kind kind, TSNode node, Captures captures);
     ...
     */
    protected boolean accept(final Element.Kind kind, final TSNode node, final Captures captures) {
        return true;
    }
    ...
    private List<Detected> detect() {
        final TSQuery query = this.query;
    ...
    private void build(final List<Detected> found) {
        ...
        for (final Detected d : found) {
            ...
            final Element.Kind kind = refineKind(d.kind, d.node, d.captures);
            ...
            final boolean dropped = !accept(kind, d.node, d.captures) || (parent != null && (parent.dropped
                    || parent.kind == Element.Kind.METHOD || parent.kind == Element.Kind.FIELD));
            Element element = null;
            if (!dropped) {
                final Element parentElement = parent == null ? root : parent.element;
                final Signature sig = signature(kind, d.node, d.captures);
                if (d.scope) {
                    element = element(kind, sig, parentElement, (TSNode) null);
                } else if (d.endNode != null) {
                    element = element(kind, sig, parentElement, d.node, d.endNode);
                } else {
                    element = element(kind, sig, parentElement, contentNode(d.node));
                }
            }
