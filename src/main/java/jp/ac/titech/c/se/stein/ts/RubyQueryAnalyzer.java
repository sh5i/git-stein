...
public class RubyQueryAnalyzer extends QueryAnalyzer {
    ...
    protected String name(final ElementKind kind, final TSNode node, final Captures captures) {
        ...
        return label + "(" + signature(node.getChildByFieldName("parameters")) + ")";
    }
    ...
    private void prune(final Element element) {
        ...
        element.getChildren().removeIf(e -> e.getKind() == ElementKind.FIELD && !isConstantAssignment(e));
    }
    ...
    private boolean isConstantAssignment(final Element field) {
        ...
        final TSNode left = field.node.getChildByFieldName("left");
