...
/**
 * Prunes empty commits: a commit whose tree equals its first parent's tree is dropped
 * (its children splice onto that parent), so a chain of such commits collapses.
 * This is the generic prune-empty that turns a content filter into a filter-repo-style pass
 * when composed (e.g. {@code @subdir X @skip-empty}).
...
public class SkipEmpty extends CommitDropper {
    ...
    protected boolean shouldDrop(final RevCommit commit, final Context c) {
        // A non-merge commit that introduced no change from its first parent; merges and roots are kept.
        return commit.getParentCount() == 1
                && commit.getTree().equals(commit.getParent(0).getTree());
    }
}
