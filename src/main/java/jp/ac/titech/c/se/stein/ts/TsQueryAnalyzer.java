...
/**
 * A query-based reimplementation of {@link TsAnalyzer}. TypeScript is a superset of JavaScript, so this
 * reuses {@link JsQueryAnalyzer}'s naming and binding logic and supplies a query that adds the
 * TypeScript constructs: abstract classes and their members, interfaces (classes whose method and
 * property signatures are members), enums (classes), type aliases (fields), and namespaces
 * ({@code internal_module}/{@code module}) as {@code @scope}. Signatures are rendered by a
 * {@link TsAnalyzer}, so parameter types are stripped, leaving parameter names.
 */
public class TsQueryAnalyzer extends JsQueryAnalyzer {
    ...
    public TsQueryAnalyzer(final String filename, final SourceText text, final TSNode treeRoot) {
        super(filename, text, treeRoot);
    }

    @Override
    protected JsAnalyzer createRenderer() {
        return new TsAnalyzer(filename, text, treeRoot);
    }
